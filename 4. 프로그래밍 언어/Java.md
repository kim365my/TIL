...
- 오버라이드 / 오버로딩
	- 오버라이드(override) 
		...
		- <mark class="hltr-purple">메서드의 선언부는 동일, 구현부만 재정의</mark><!--ID: 1684538923319-->
	- 오버로딩(overloading) : 객체 지향에서 같은 이름의 메소드에 ==다른 매개변수==를 작성한 메소드를 만든다는 의미 <!--ID: 1684538923321-->
- 레퍼 클래스(Wraper Class) : 자바에서 자료형은 객체가 아니니까 객체지향에 위배되기에 자료형을 객체로 감싸는 느낌으로 만든 클래스를 레퍼 클래스라고 함
