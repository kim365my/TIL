...
- **form태그의 속성**
	...
	- `enctype` 속성 : 폼 데이터가 서버로 제출될 때 해당 데이터가 인코딩되는 방법을 명시
		- [!] method 속성값이 post인 경우만 사용가능
		- [?] 서버에 제출할 때 인코딩이 어떻게 되는 걸까? #질문 
		- application/x-www-form-urlencoded
			- [기본 값] 모든 문자들은 서버로 보내기 전에 인코딩됨을 명시함.
		- multipart/form-data
			- 모든 문자를 인코딩하지 않음을 명시함. 이 방식은 요소가 <mark class="hltr-purple">파일이나 이미지를 서버로 전송할 때 주로 사용</mark>함.
		- text/plain 
