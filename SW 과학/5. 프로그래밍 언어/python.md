...
'''
station에 할당된 사당, 신도림, 인천공항 값에 변호를 순서대로 부여해서 (1,2,3으로) 
처음 실행했을 때 1번 출력, 두번째 실행했을 때 2번 출력... 같이 못하나??
'''
```
...
파이썬은 쉬워요
"""
print(sentence3) #따옴표 3개(""")를 통해 위처럼 길게 글을 표시할 수 있음
```
...
### 튜플 : 리스트와 다르게 내용의 변경이나 추가를 할 수 없지만 속도가 리스트보다 빠름.
```python
# 튜플, 리스트와 다르게 내용의 변경이나 추가를 할 수 없지만 속도가 리스트보다 빠름. 
menu = "돈가스", "치즈까스"
